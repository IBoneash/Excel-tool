...
# class input
class Input(object):
    def __init__(self, xh='', dy='', gg='', ll='', l='', tj='', tjv=0, tjl=None):
        self.xh = xh
        self.dy = dy
        self.gg = gg
        self.ll = ll
...
if __name__ == '__main__':
    try:
        while True:
            ...
            tongjia = raw_input('Input price of copper(split by /): ')
            tj = tongjia.split('/')
            for p in tj:
                data.tjl.append(float(p))

            length_tmp = ''
            for data.tj in data.tjl:
                ...
                if data.l == '':
                    data.l = raw_input('Input letters: ')
                    data.ll = data.l
                else:
                    data.l = raw_input('Input letters: %s ?' % data.ll)
                    if data.l != '':
                        data.ll = data.l
                    else:
                        data.l = data.ll
                ...
                for x in range(0, xl.open().nsheets):
                    ...
                    for i in range(0, sh.nrows):
                        ...
                        if data.xh in row_data and data.gg in row_data and (
                            ...
                            if length_tmp == '':
                                length_tmp = raw_input('Enter length = ')
                                length = int(length_tmp)
                            else:
                                length_tmp = raw_input('Enter length = %s ?' % length)
                                if length_tmp != '':
                                    length = length_tmp
                                else:
                                    length_tmp = length
